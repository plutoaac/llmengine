...
Status PagedKVCache::init_cuda(int num_layers, int num_kv_heads, int head_dim,
    ...
    if (num_layers <= 0 || num_kv_heads <= 0 || head_dim <= 0 ||
        block_size <= 0 || max_blocks <= 0) {
        return Status::invalid_argument(
            "PagedKVCache CUDA init requires positive parameters");
    }
    ...
    const size_t block_bytes = static_cast<size_t>(block_size_) * kv_hidden() * sizeof(float);
    const size_t total_bytes = static_cast<size_t>(max_blocks_) * block_bytes;

    layers_.resize(static_cast<size_t>(num_layers));
    for (auto& layer : layers_) {
        err = cudaMalloc(&layer.cuda_k, total_bytes);
        if (err != cudaSuccess) {
            (void)release();
            return Status::runtime_error("cudaMalloc K failed: " + std::string(cudaGetErrorString(err)));
        }
        err = cudaMalloc(&layer.cuda_v, total_bytes);
        if (err != cudaSuccess) {
            (void)release();
            return Status::runtime_error("cudaMalloc V failed: " + std::string(cudaGetErrorString(err)));
        }
        err = cudaMemset(layer.cuda_k, 0, total_bytes);
        if (err != cudaSuccess) {
            (void)release();
            return Status::runtime_error("cudaMemset K failed");
        }
        err = cudaMemset(layer.cuda_v, 0, total_bytes);
        if (err != cudaSuccess) {
            (void)release();
            return Status::runtime_error("cudaMemset V failed");
        }
    }
    ...
    for (int b = max_blocks_ - 1; b >= 0; --b)
        free_blocks_.push_back(b);
    sequences_.clear();
    cuda_block_tables_.clear();
    cuda_block_table_sizes_.clear();
    return Status::make_ok();
}

float* PagedKVCache::cuda_k_data(int layer) {
    return layers_[static_cast<size_t>(layer)].cuda_k;
}

float* PagedKVCache::cuda_v_data(int layer) {
    return layers_[static_cast<size_t>(layer)].cuda_v;
}

bool PagedKVCache::is_cuda() const {
    return cuda_device_ >= 0 && num_layers() > 0 && layers_[0].cuda_k != nullptr;
}

Status PagedKVCache::upload_block_table(int sequence_id) {
    if (sequence_id < 0 || static_cast<size_t>(sequence_id) >= sequences_.size())
        return Status::not_found("sequence not found for block table upload");

    const auto& seq = sequences_[static_cast<size_t>(sequence_id)];
    const size_t count = seq.blocks.size();
    if (count == 0) return Status::make_ok();

    if (static_cast<size_t>(sequence_id) >= cuda_block_tables_.size()) {
        cuda_block_tables_.resize(static_cast<size_t>(sequence_id) + 1, nullptr);
        cuda_block_table_sizes_.resize(static_cast<size_t>(sequence_id) + 1, 0);
    }

    if (cuda_block_tables_[static_cast<size_t>(sequence_id)] && cuda_block_table_sizes_[static_cast<size_t>(sequence_id)] >= count)
        ; // already allocated, reuse
    else {
        if (cuda_block_tables_[static_cast<size_t>(sequence_id)])
            cudaFree(cuda_block_tables_[static_cast<size_t>(sequence_id)]);
        cudaError_t err = cudaMalloc(&cuda_block_tables_[static_cast<size_t>(sequence_id)],
                                       count * sizeof(int));
        if (err != cudaSuccess)
            return Status::runtime_error("cudaMalloc block table failed: " + std::string(cudaGetErrorString(err)));
        cuda_block_table_sizes_[static_cast<size_t>(sequence_id)] = count;
    }

    cudaError_t err = cudaMemcpy(cuda_block_tables_[static_cast<size_t>(sequence_id)],
                                   seq.blocks.data(), count * sizeof(int),
                                   cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
        return Status::runtime_error("cudaMemcpy block table failed: " + std::string(cudaGetErrorString(err)));
    return Status::make_ok();
}

const int* PagedKVCache::device_block_table(int sequence_id) const {
    if (sequence_id < 0 || static_cast<size_t>(sequence_id) >= cuda_block_tables_.size())
        return nullptr;
    return cuda_block_tables_[static_cast<size_t>(sequence_id)];
}

Status PagedKVCache::write_tokens_cuda(int sequence_id, int layer, int start_pos,
                                        const float* k, const float* v, int token_count) {
    if (!k || !v)
        return Status::invalid_argument("write_tokens_cuda requires non-null K/V");
    ...
    const size_t block_stride = static_cast<size_t>(block_size_) * row_floats;

    auto& layer_ref = layers_[static_cast<size_t>(layer)];
    float* cuda_k = layer_ref.cuda_k;
    float* cuda_v = layer_ref.cuda_v;

    for (int t = 0; t < token_count; ++t) {
        const int pos = start_pos + t;
        auto block = physical_block_for_position(sequence_id, pos);
        if (!block) return block.error();
        const int offset = pos % block_size_;
        const size_t device_offset = static_cast<size_t>(*block) * block_stride +
                                      static_cast<size_t>(offset) * row_floats;

        cudaError_t err = cudaMemcpy(cuda_k + device_offset,
                                       k + static_cast<size_t>(t) * row_floats,
                                       row_bytes, cudaMemcpyDeviceToDevice);
        if (err != cudaSuccess)
            return Status::runtime_error("cudaMemcpy K to paged cache failed");
        err = cudaMemcpy(cuda_v + device_offset,
                         v + static_cast<size_t>(t) * row_floats,
                         row_bytes, cudaMemcpyDeviceToDevice);
        if (err != cudaSuccess)
            return Status::runtime_error("cudaMemcpy V to paged cache failed");
    }
...
#endif // MINILLM_ENABLE_CUDA

void PagedKVCache::reset() {
#if defined(MINILLM_ENABLE_CUDA)
    for (auto& layer : layers_) {
        if (layer.cuda_k)
            cudaMemset(layer.cuda_k, 0,
                       static_cast<size_t>(max_blocks_) * block_size_ * kv_hidden() * sizeof(float));
        if (layer.cuda_v)
            cudaMemset(layer.cuda_v, 0,
                       static_cast<size_t>(max_blocks_) * block_size_ * kv_hidden() * sizeof(float));
    }
#endif
    for (auto& layer : layers_) {
        if (!layer.k.empty()) std::fill(layer.k.begin(), layer.k.end(), 0.0f);
        if (!layer.v.empty()) std::fill(layer.v.begin(), layer.v.end(), 0.0f);
    }
    block_used_.assign(static_cast<size_t>(max_blocks_), false);
    free_blocks_.clear();
    free_blocks_.reserve(static_cast<size_t>(max_blocks_));
...
Status PagedKVCache::release() {
#if defined(MINILLM_ENABLE_CUDA)
    for (auto& layer : layers_) {
        if (layer.cuda_k) { cudaFree(layer.cuda_k); layer.cuda_k = nullptr; }
        if (layer.cuda_v) { cudaFree(layer.cuda_v); layer.cuda_v = nullptr; }
    }
    for (auto* bt : cuda_block_tables_)
        if (bt) cudaFree(bt);
    cuda_block_tables_.clear();
    cuda_block_table_sizes_.clear();
    cuda_device_ = 0;
...
Status PagedKVCache::free_sequence(int sequence_id) {
    ...
    if (static_cast<size_t>(sequence_id) < cuda_block_tables_.size()) {
        if (cuda_block_tables_[static_cast<size_t>(sequence_id)]) {
            cudaFree(cuda_block_tables_[static_cast<size_t>(sequence_id)]);
            cuda_block_tables_[static_cast<size_t>(sequence_id)] = nullptr;
        }
        cuda_block_table_sizes_[static_cast<size_t>(sequence_id)] = 0;
    }
#endif
    return Status::make_ok();
}
...
int PagedKVCache::sequence_capacity(int sequence_id) const {
    if (sequence_id < 0 || static_cast<size_t>(sequence_id) >= sequences_.size())
        return 0;
    return static_cast<int>(sequences_[static_cast<size_t>(sequence_id)].blocks.size()) * block_size_;
}
...
std::expected<int, Status> PagedKVCache::physical_block_for_position(
    ...
    if (logical_block < 0 || static_cast<size_t>(logical_block) >= seq.blocks.size())
        return std::unexpected(Status::out_of_range("token position outside block table"));
...
std::expected<float*, Status> PagedKVCache::mutable_token_ptr(
    ...
    if (layer < 0 || layer >= num_layers())
        return std::unexpected(Status::out_of_range("layer index out of range"));
...
std::expected<const float*, Status> PagedKVCache::token_ptr(
    ...
    if (kv_head < 0 || kv_head >= num_kv_heads_)
        return std::unexpected(Status::out_of_range("KV head index out of range"));
    if (token_pos < 0 || token_pos >= sequence_length(sequence_id))
        return std::unexpected(Status::out_of_range("token position outside sequence length"));
...
Status PagedKVCache::write_tokens(int sequence_id, int layer, int start_pos,
                                  const float* k, const float* v, int token_count) {
    if (!k || !v)
        return Status::invalid_argument("write_tokens requires non-null K/V");
    if (start_pos < 0 || token_count < 0)
        return Status::invalid_argument("start_pos and token_count must be non-negative");
    if (layer < 0 || layer >= num_layers())
        return Status::out_of_range("layer index out of range");
    if (token_count == 0)
        return Status::make_ok();
...
Status PagedKVCache::append_tokens(int sequence_id, int layer,
                                    const float* k, const float* v, int token_count) {
    return write_tokens(sequence_id, layer, sequence_length(sequence_id), k, v, token_count);
}
